...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls:
      ...
      if tmp_val == 1:
        flag = True
      total += tmp_val
    return total if not flag else 1
    # END PROBLEM 1
...
def swine_align(player_score, opponent_score):
    ...
    return opponent_score > 0 and player_score > 0 and gcd(player_score, opponent_score) >= 10
    # END PROBLEM 4a
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(s0, s1):
      s_now = s0 if who == 0 else s1
      if s_now - last_score > running_high:
        print(f'{s_now - last_score} point(s)! The most yet for Player {who}')
        return announce_highest(who, s_now, s_now - last_score)
      return announce_highest(who, s_now, running_high)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    res = []
    for i in range(1, 11):
      tmp = make_averaged(roll_dice, trials_count=1000)
      res.append(tmp(i,dice))
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return 0 if extra_turn(score + free_bacon(opponent_score),opponent_score) else bacon_strategy(score, opponent_score, cutoff, num_rolls)  # Replace this statement
    # END PROBLEM 11
